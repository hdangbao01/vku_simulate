...
function NavItem({ item }) {
    ...
    >
        {drops ? <>
            <p key={label} className={`text-black cursor-pointer hover:text-blue-600 ml-8${active ? ' text-blue-600' : ''}`}>
                {label}
            </p>
            ...
            :
            <Link to={link}>
                <p className={`text-black cursor-pointer hover:text-blue-600 ml-8${active ? ' text-blue-600' : ''}`}>
                    {label}
                </p>
            </Link>
        }
